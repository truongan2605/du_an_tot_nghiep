        ...
        <form method="GET" action="{{ route('staff.rooms') }}" class="row g-2 align-items-end">
            ...
            <div class="col-md-3">
                ...
                <select name="trang_thai" class="form-select">
                    ...
                    <option value="dang_su_dung" {{ request('trang_thai') == 'dang_su_dung' ? 'selected' : '' }}>Đang sử
                        dụng
                    </option>
                    <option value="dang_don_dep" {{ request('trang_thai') == 'dang_don_dep' ? 'selected' : '' }}>Đang dọn
                        dẹp
                    </option>
                </select>
            </div>
            <div class="col-md-3">
                ...
                <select name="trang_thai_booking" class="form-select">
                    <option value="">-- Tất cả --</option>
                    <option value="dang_cho" {{ request('trang_thai_booking') == 'dang_cho' ? 'selected' : '' }}>Chờ Xác
                        Nhận
                    </option>
                    <option value="da_xac_nhan" {{ request('trang_thai_booking') == 'da_xac_nhan' ? 'selected' : '' }}>Đã
                        Xác Nhận
                    </option>
                    <option value="da_gan_phong" {{ request('trang_thai_booking') == 'da_gan_phong' ? 'selected' : '' }}>Đã
                        Gán
                        Phòng</option>
                </select>
            </div>
                        ...
                        <td class="text-center">
                            @if ($room->trang_thai == 'trong' && $room->datPhongItem && $room->datPhongItem->datPhong->trang_thai == 'dang_cho')
                                <a href="{{ route('staff.assign-rooms', $room->datPhongItem->datPhong->id) }}"
                                    class="btn btn-sm btn-warning me-1">Gán Phòng</a>
                            @elseif($room->trang_thai == 'dang_su_dung')
