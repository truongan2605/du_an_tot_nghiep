            ...
            <div class="card border-0 shadow-sm chart-card" style="overflow: hidden;">
                <div class="card-header bg-white border-bottom py-3">
                    <div class="d-flex flex-column gap-3">
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                            ...
                            {{-- Form lọc --}}
                            <form method="GET" action="{{ route('staff.index') }}" class="d-flex flex-column flex-sm-row gap-2 align-items-start align-items-sm-end" id="revenueFilterForm" onsubmit="return validateDateRange()">
                        ...
                        <ul class="nav nav-pills flex-wrap mb-0" id="revenueTabs" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link {{ !request()->has('start_date') ? 'active' : '' }}" id="today-tab" data-bs-toggle="pill" data-bs-target="#today" type="button" role="tab">
                            ...
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="room-tab" data-bs-toggle="pill" data-bs-target="#room" type="button" role="tab">
                                    <i class="bi bi-door-open me-1"></i>Theo Phòng
                                </button>
                            </li>
                            @if(request()->has('start_date') && request()->has('end_date'))
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active" id="custom-tab" data-bs-toggle="pill" data-bs-target="#custom" type="button" role="tab">
                ...
                <div class="card-body p-3" style="overflow-x: hidden;">
                    <div class="tab-content" id="revenueTabContent" style="width: 100%; max-width: 100%;">
                        {{-- Tab Hôm Nay --}}
                        <div class="tab-pane fade {{ !request()->has('start_date') ? 'show active' : '' }}" id="today" role="tabpanel">
                            <div class="row g-3">
                                <div class="col-lg-8">
                                    <div class="mb-2">
                                        ...
                                        <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center gap-3">
                                            <div class="flex-grow-1 w-100">
                                                <div class="display-4 fw-bold text-primary mb-0" style="word-break: break-word;">
                                    ...
                                    <div class="card bg-light border-0 h-100">
                                        <div class="card-body p-3">
                                            ...
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                <strong class="text-success fs-5">{{ number_format($todayNetRevenue ?? 0, 0) }}đ</strong>
                                            </div>
                                            @if($todayRefund > 0)
                            ...
                            <div class="card-body p-3">
                                ...
                                <div style="max-height: 300px; overflow-y: auto;">
                                    @forelse(collect($roomRevenueMonth ?? [])->take(10) as $room)
                                        <div class="d-flex justify-content-between align-items-center mb-3 pb-3 border-bottom">
                                            <div>
                                                <div class="fw-semibold text-dark">{{ $room->ma_phong }}</div>
                                                <small class="text-muted">{{ $room->loai_phong }}</small>
                                                <br>
                                                <small class="text-muted">{{ $room->booking_count ?? 0 }} đơn</small>
                                            </div>
                                            <div class="text-end">
                                                <strong class="text-success d-block">{{ number_format($room->revenue ?? 0, 0) }}đ</strong>
                                                <small class="text-muted">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover mb-0">
                                        ...
                                        <tbody>
                                            @php
                                                $allRooms = collect();
                                                if (!empty($roomRevenueToday)) $allRooms = $allRooms->merge($roomRevenueToday);
                                                if (!empty($roomRevenueWeek)) $allRooms = $allRooms->merge($roomRevenueWeek);
                                                if (!empty($roomRevenueMonth)) $allRooms = $allRooms->merge($roomRevenueMonth);
                                            ...
                                            @forelse($allRooms->sortByDesc(function($room) use ($roomRevenueMonth) {
                                                ...
                                                @php
                                                    $todayRev = $roomRevenueToday->firstWhere('id', $room->id)->revenue ?? 0;
                                                    $weekRev = $roomRevenueWeek->firstWhere('id', $room->id)->revenue ?? 0;
                                                    $monthRev = $roomRevenueMonth->firstWhere('id', $room->id)->revenue ?? 0;
                                                    $bookingCount = $roomRevenueMonth->firstWhere('id', $room->id)->booking_count ?? 0;
                                                    $roomStatus = $roomRevenueMonth->firstWhere('id', $room->id)->trang_thai ?? $room->trang_thai ?? 'trong';
                                                @endphp
                                                <tr>
                                                    <td class="ps-3">
                                                    ...
                                                    <td>
                                                        <small class="text-muted">{{ $room->loai_phong ?? 'N/A' }}</small>
                                                    </td>
                                                    <td class="text-center">
                                                        <span class="badge bg-success-subtle text-success">
                                                            {{ number_format($todayRev, 0) }}đ
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-12">
                            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2 mb-2">
                                <div>
                                    <h6 class="text-muted mb-1 small">Doanh thu từ {{ $customRangeLabel ?? '' }}</h6>
                                    <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center gap-2">
                                        <div class="display-4 fw-bold text-primary mb-0" style="font-size: clamp(1.5rem, 4vw, 2.5rem); word-break: break-word; line-height: 1.2;">
                                            {{ number_format($customRevenue ?? 0, 0) }}<small class="fs-6">đ</small>
                                        </div>
                                        <div class="badge bg-purple-subtle text-purple fs-6 px-3 py-2" style="background-color: #e7d2ff; color: #6f42c1; white-space: nowrap;">
                                            <i class="bi bi-calendar-range me-1"></i>Tùy chỉnh
                                        </div>
                                    </div>
                                    <div class="mt-2">
                                        @if($customRefund > 0)
                                            <span class="badge bg-warning-subtle text-warning me-2">
                                                <i class="bi bi-arrow-counterclockwise me-1"></i>Hoàn: {{ number_format($customRefund, 0) }}đ
                                            </span>
                                        @endif
                                        <span class="badge bg-success-subtle text-success">
                                            <i class="bi bi-check-circle me-1"></i>Lợi nhuận: {{ number_format($customNetRevenue ?? 0, 0) }}đ
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card bg-light border-0 shadow-sm h-100">
                                <div class="card-body p-3">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-3 pb-2 border-bottom">
                                        ...
                                        <strong class="text-success fs-6">{{ number_format($customPaid ?? 0, 0) }}đ</strong>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <span class="fw-semibold">Lợi nhuận</span>
                                        <strong class="text-success fs-5">{{ number_format($customNetRevenue ?? 0, 0) }}đ</strong>
                                    </div>
                                    @if($customRefund > 0)
                                        <div class="mt-2 pt-2 border-top">
                                            <small class="text-muted d-flex align-items-center">
                                                <i class="bi bi-info-circle me-1"></i>
                                                Đã trừ hoàn tiền: <strong class="text-warning ms-1">{{ number_format($customRefund, 0) }}đ</strong>
                                            </small>
                                        </div>
                                    @endif
                                </div>
...
/* Fix all columns */
.col-12, .col-lg-8, .col-lg-4, .col-md-6, .col-6 {
    max-width: 100%;
    box-sizing: border-box;
}

/* Prevent text overflow */
h5, h6, .fw-bold, .fw-semibold {
