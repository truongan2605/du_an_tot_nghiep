                    ...
                    <div class="offcanvas-body p-3 p-lg-0">
                        <div class="card bg-light w-100">
                            <div class="position-absolute top-0 end-0 p-3">
                                <a href="{{ route('account.settings') }}" class="text-primary-hover" data-bs-toggle="tooltip" data-bs-title="Edit profile">
                            ...
                            <div class="card-body p-3">
                                <div class="text-center mb-3">
                                    ...
                                    <a href="mailto:{{ $user->email }}" class="text-reset text-primary-hover small">{{ $user->email }}</a>
                                    <hr>
                                </div>

                                <ul class="nav nav-pills-primary-soft flex-column">
                                    <li class="nav-item">
                                        <a class="nav-link" href="{{ route('account.settings') }}"><i class="bi bi-person fa-fw me-2"></i>My Profile</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link active" href="{{ route('account.booking.index') }}"><i class="bi bi-ticket-perforated fa-fw me-2"></i>My Bookings</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" href="{{ route('account.wishlist') }}"><i class="bi bi-heart fa-fw me-2"></i>Wishlist</a>
                                    </li>
                                    <li class="nav-item">
                                        <form method="POST" action="{{ route('logout') }}">
                                            @csrf
                                            <button type="submit" class="btn nav-link text-start text-danger bg-danger-soft-hover w-100">
                        ...
                        <div class="tab-content p-2 p-sm-4" id="nav-tabContent">
                            ...
                            <div class="tab-pane fade show active" id="tab-1">
                                ...
                                @forelse($upcoming as $b)
                                    @php
                                        $meta = is_array($b->snapshot_meta) ? $b->snapshot_meta : (json_decode($b->snapshot_meta, true) ?: []);
                                        $roomsCount = $meta['rooms_count'] ?? ($b->datPhongItems->sum('so_luong') ?: 1);
                                    ...
                                    <div class="card border mb-4">
                                        <div class="card-header border-bottom d-md-flex justify-content-md-between align-items-center">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="ms-2">
                                                    ...
                                                    <ul class="nav nav-divider small">
                                                        ...
                                                        <li class="nav-item">Total: {{ number_format($b->snapshot_total ?? $b->tong_tien ?? 0, 0, ',', '.') }} VND</li>
                                                    </ul>
                                                </div>
                                            </div>
                                            ...
                                            <div class="mt-2 mt-md-0 text-end">
                                                ...
                                                <div class="mt-2">
                                                    <a href="{{ route('account.booking.show', $b->id) }}" class="btn btn-primary-soft mb-0">Manage Booking</a>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="card-body">
                                            <div class="row g-3">
                                                <div class="col-sm-6 col-md-4">
                                                    <span>Check in</span>
                                                    <h6 class="mb-0">{{ optional($b->ngay_nhan_phong)->format('D, d M Y') }}</h6>
                                                </div>

                                                <div class="col-sm-6 col-md-4">
                                                    <span>Check out</span>
                                                    <h6 class="mb-0">{{ optional($b->ngay_tra_phong)->format('D, d M Y') }}</h6>
                                                </div>

                                                <div class="col-md-4">
                                                    <span>Contact</span>
                                                    <h6 class="mb-0">{{ $b->contact_name ?? $user->name ?? $user->email }}</h6>
                                                </div>
                                            </div>
                                            ...
                                            @if($b->datPhongItems && $b->datPhongItems->count())
                                                <ul class="list-unstyled mb-0">
                                                    @foreach($b->datPhongItems as $it)
                                                        @php
                                                            $roomName = $it->phong && isset($it->phong->name) ? $it->phong->name
                                                                        : ($it->loai_phong && isset($it->loai_phong->name) ? $it->loai_phong->name
                                                        ...
                                                        <li class="mb-1">
                                                            ...
                                                            {{ $roomName }}
                                                            @if(isset($it->so_dem))
                                                                <small class="text-muted"> — {{ $it->so_dem }} night(s)</small>
                                                            @endif
                                                        </li>
                                                    @endforeach
                                                </ul>
                                    ...
                                    <div class="card border mb-3">
                                        <div class="card-header d-flex justify-content-between align-items-center">
                                            <div>
                                                ...
                                                <small class="text-muted">Cancelled at {{ optional($b->updated_at)->format('d M Y H:i') }}</small>
                                            </div>
                                            <div class="text-end">
                                                <span class="badge {{ $badge }}">{{ $label }}</span>
                                            </div>
                                        ...
                                        <div class="card-body">
                                            <div class="row">
                                                ...
                                                <div class="col-md-4">
                                                    <span>Total</span>
                                                    <h6 class="mb-0">{{ number_format($b->snapshot_total ?? $b->tong_tien ?? 0,0,',','.') }} VND</h6>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                @forelse($completed as $b)
                                    ...
                                    <div class="card border mb-3">
                                        <div class="card-header d-flex justify-content-between align-items-center">
                                            <div>
                                                <h6 class="mb-0">{{ $b->ma_tham_chieu }}</h6>
                                                <small class="text-muted">Completed at {{ optional($b->updated_at)->format('d M Y H:i') }}</small>
                                            </div>
                                            <div class="text-end">
                                                <span class="badge {{ $badge }}">{{ $label }}</span>
                                            </div>
