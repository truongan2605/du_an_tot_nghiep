...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-1">
                <i class="fas fa-edit me-2 text-primary"></i>Chỉnh sửa thông báo nội bộ
            </h1>
            <p class="text-muted mb-0">Cập nhật thông tin thông báo nội bộ</p>
        </div>
        <div class="btn-group">
            <a href="{{ route('admin.internal-notifications.index') }}" class="btn btn-secondary">
                <i class="fas fa-arrow-left me-1"></i>Quay lại
            </a>
            <a href="{{ route('admin.internal-notifications.show', $notification) }}" class="btn btn-outline-primary">
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.internal-notifications.update', $notification) }}">
                        @csrf
                        @method('PUT')
                                ...
                                <select class="form-select" id="nguoi_nhan_id" name="nguoi_nhan_id">
                                    ...
                                    @foreach($staff as $person)
                                        <option value="{{ $person->id }}" 
                                                {{ old('nguoi_nhan_id', $notification->nguoi_nhan_id) == $person->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control" id="ten_template" name="ten_template" 
                                   value="{{ old('ten_template', $notification->ten_template) }}" placeholder="Ví dụ: system_announcement, meeting_reminder" required>
                            @error('ten_template')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea class="form-control" id="payload" name="payload" rows="8" 
                                      placeholder='{"title": "Tiêu đề thông báo", "message": "Nội dung chi tiết", "link": "/admin/dashboard"}'>{{ old('payload', $notification->payload ? json_encode($notification->payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : '') }}</textarea>
                            <div class="form-text">
                                <strong>Ví dụ JSON:</strong><br>
                                <code>{"title": "Họp team", "message": "Họp team lúc 14:00 ngày mai tại phòng họp A", "link": "/admin/meetings"}</code>
                            </div>
                            @error('payload')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('admin.internal-notifications.show', $notification) }}" class="btn btn-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    templateSelect.addEventListener('change', function() {
        ...
        if (selectedTemplate) {
            window.notificationTemplates.loadTemplateToTextarea('internal', selectedTemplate, 'payload');
        }
    });
    ...
    templateInput.addEventListener('input', function() {
        ...
        if (template && !payloadTextarea.value) {
            ...
            const templates = window.notificationTemplates.getTemplatesByType('internal');
            if (templates[template]) {
                window.notificationTemplates.loadTemplateToTextarea('internal', template, 'payload');
            }
        }
    });
